...
void two_layer::read_file(const char **test){
	...
	sort (vases_v_x.begin(),vases_v_x.end(),compare);
	sort (vases_v_y.begin(),vases_v_y.end(),compare);
...
void two_layer::find_bottom_gridline_x(){
	pair < double , double > temp_xy;
	temp_xy.first = -1;
	temp_xy.second = -1;
	vector < pair < double , double > > x_obs;
	vector < pair < double , double > > y_obs;
	vector <double> via_center_x;
	double temp_line;
	for (int i = 0 ; i < vases_v_x.size() ; ){ // i++
		if (vases_v_x[i].state == 'b'){
			temp_xy.first = vases_v_x[i].xy;
			i++;
		}
		else{ //e
			temp_xy.second = vases_v_x[i].xy;
			
			if ( i == vases_v_x.size()-1){
				x_obs.push_back(temp_xy);
				break;
			}
			else if (vases_v_x[i+1].state == 'b'){
				x_obs.push_back(temp_xy);
			}
			i++;
		}
	...
	for (int i = 0 ; i < vases_v_y.size() ; ){ // i++
		if (vases_v_y[i].state == 'b'){
			temp_xy.first = vases_v_y[i].xy;
			i++;
		}
		else{ //e
			temp_xy.second = vases_v_y[i].xy;
			
			if ( i == vases_v_y.size()-1){
				y_obs.push_back(temp_xy);
				break;
			}
			else if (vases_v_y[i+1].state == 'b'){
				y_obs.push_back(temp_xy);
			}
			i++;
		}
	...
	/////
	cout << "x_bottom_obs" << endl;
	for (int i = 0; i < x_obs.size() ; i++){
		cout << "( " << x_obs[i].first << "," << x_obs[i].second << ") "; 
	}
	cout << endl;
	cout << "y_bottom_obs" << endl;
	for (int i = 0; i < y_obs.size() ; i++){
		cout << "( " << y_obs[i].first << "," << y_obs[i].second << ") "; 
	}
	cout << endl;
	...
	for (int i = 0; i < x_obs.size() ; i++ ){
		temp_line = (x_obs[i].first + x_obs[i].second ) / 2;
		via_center_x.push_back(temp_line);
		x_bottom_line.push_back(temp_line);
	}
	cout << "via_center_x: ";
	for (int i = 0; i < via_center_x.size() ; i++ ){
		cout << via_center_x[i] << " " ;
	}
	cout << endl; 
	////new
	for (int i = 0; i < via_center_x.size() ; i++ ){
		temp_line = via_center_x[i] + layers[1].spacing + layers[1].width;
		if ( i != via_center_x.size()-1){		
			while( temp_line <= via_center_x[i+1]-layers[1].spacing-layers[1].width ){
				x_bottom_line.push_back(temp_line);
				temp_line = temp_line+layers[1].spacing + layers[1].width;
			}
			
			if ( i == 0){
				temp_line = via_center_x[i] - layers[1].spacing - layers[1].width;
				while( temp_line >= Dsgn_ll.first + 0.5*layers[1].width ){ //via_center_x[i+1].first-layers[0].spacing-0.5*layers[0].width
					x_bottom_line.push_back(temp_line);
					temp_line = temp_line - layers[1].spacing - layers[1].width;				
				}
			} 
			...
			while( temp_line <= Dsgn_ru.first - 0.5*layers[1].width){
				x_bottom_line.push_back(temp_line);
				temp_line = temp_line + layers[1].spacing + layers[1].width;
			}
		}
...
void two_layer::copy_gird(){
	y_bottom_line = y_line;
	for (int i = 0; i < y_bottom_line.size();i++){
		cout << y_bottom_line[i] << " ";
	}
	cout << endl;
}
...
void two_layer::find_path(){
	...
	for (int i = 0; i < nets.size();i++){
		...
		if (shapes[start].center.first < shapes[end].center.first){
			for (int j = 0; j < x_line.size();j++){
				if (shapes[end].center.first == x_line[j]){
					if ( shapes[start].center.first <= x_line[j-1]){
						temp_x = j-1;
					}
					else{
						temp_x = j;
			...
			////
			check = check_path(1, temp_y-counter, &shapes[start].center.first, &x_line[temp_x]);
			while ( ! check ){
				counter++;
				check = check_path(1, temp_y-counter, &shapes[start].center.first, &x_line[temp_x]);
			}
			temp_segment.segment.first = shapes[start].center.first;
			...
			////
			check = check_path(1, temp_y-counter, &x_line[temp_x], &shapes[start].center.first);
			while ( ! check ){
				counter++;
				check = check_path(1, temp_y-counter, &x_line[temp_x], &shapes[start].center.first);
			}
			temp_segment.segment.first = x_line[temp_x];
			temp_segment.segment.second = shapes[start].center.first;
		}
		...
		for (int j = 0; j < x_bottom_line.size();j++){
			if ( x_bottom_line[j] == shapes[start].center.first){
				temp_x = j;
			}
		}
		temp_segment.segment.first = shapes[start].center.second;
...
void two_layer::gnuplot_bottom_pic(const char *output){
	...
	for (int i =0; i < x_bottom_line.size(); i++){
		gnuCmd_out << "set arrow from "<<x_bottom_line[i]<<","<<Dsgn_ll.second<<" to "<<x_bottom_line[i]<<","<<Dsgn_ru.second<<" nohead lc 4 lw 1"<<endl;
	}
	
	for (int i =0; i < y_bottom_line.size(); i++){
		gnuCmd_out << "set arrow from "<<Dsgn_ll.first<<","<<y_bottom_line[i]<<" to "<<Dsgn_ru.first<<","<<y_bottom_line[i]<<" nohead lc 5 lw 1"<<endl;
	}
